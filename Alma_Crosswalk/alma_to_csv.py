...
for record in marc_dicts:
    ...
    print(record.get("041"),record.get("001"))
...
#mu.write_json("new_marc_dicts.json", new_alma_dicts)
    ...
    if record.get("856"):
        field_856_data.append([record["001"], record["856"]])
#--------------------------------------------
...
mu.write_csv("856s_ALMA.csv", field_856_data)

dicts856 = []
full_dicts = mu.read_json("test_marc_dicts.json")
for a_dict in full_dicts:
    if "856" in a_dict.keys():
        dicts856.append(a_dict)

mu.write_json("856_dictionaries.json", dicts856)
