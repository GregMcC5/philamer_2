...
for mms_dlxs in dlxs_dict.keys():
    for mms_alma in alma_dict.keys():
        if dlxs_dict[mms_dlxs][0] == mms_alma:
            ...
            dlxs_list = dlxs_dict[mms_dlxs][10].split(":")
            if len(dlxs_list) > 2:
                dlxs_longer_list.append("language")
...
matches_doc.insert(0, headers)
mu.write_csv("matches.csv", matches_doc)
